...
and             env = Env of (string -> (term * env));
...
fun update (Env e) (x : string) (v: (term * env)) = (Env (fn y => if x = y then v else e y)); 
...
(* Interpret function *)
(* Implement this *)
fun interp (env, (AST_ID i))            = let val (e, env) = lookup(env, i) in interp(env, e)
  ...
  | interp (env, (AST_FUN (i, b)))      = RES_CLOSURE(RES_FUN(i, b), env)
  | interp (env, (AST_APP (f, p)))      =  (case interp(env, f) of 
                                             ...
                                             | RES_CLOSURE(RES_FUN(i, b) static_env) => interp(update static_env i interp(p, env), b)
                                                | _ => RES_ERROR "hahaha"
                                             )

  | interp (env, (AST_LET (i, e1, e2))) = interp(update env i (ei, env), e2)
